...
export const load: PageServerLoad = async ({ params }) => {
  ...
  if (isNaN(id)) throw error(404, 'Product not found');

  const [produits, categories, variantes, images, prixB2B] = await Promise.all([

    query(`
      SELECT
        p.id, p.nom, p.slug, p.description,
        p.reference, p.categorie_id, p.prix,
        p.unite_vente, p.stock, p.stock_minimum, p.actif
      FROM produits p
      WHERE p.id = $1
    `, [id]),
    ...
    query(`
      SELECT id, nom, reference, prix_supplement, stock, actif
      FROM variantes
      WHERE produit_id = $1
      ORDER BY id ASC
    `, [id]),
    ...
    query(`
      SELECT id, url, ordre, principale
      FROM produit_images
      WHERE produit_id = $1
      ORDER BY ordre ASC
    `, [id]),
    ...
    query(`
      SELECT
        ...
        pc.prix
      FROM prix_client pc
      JOIN types_client tc ON tc.id = pc.type_client_id
      WHERE pc.produit_id = $1
    `, [id]),

  ]);

  if (!produits[0]) throw error(404, 'Product not found');

  const prix_b2b: Record<string, number> = {};
  prixB2B.forEach((p: any) => {
    if (p.code !== 'public') {
      prix_b2b[p.code] = parseFloat(p.prix);
    }
  });
...
export const actions: Actions = {
  default: async ({ request, params }) => {
    ...
    // ─── Vérifier slug unique (sauf ce produit) ───────────
    const existing = await query(
      'SELECT id FROM produits WHERE slug = $1 AND id != $2',
      [slug, id]
    );
    ...
    for (let i = 0; i < variantsCount; i++) {
      ...
      if (!vNom) continue;
      variants.push({
        id:              data.get(`variant_id_${i}`) ? parseInt(data.get(`variant_id_${i}`) as string) : null,
        nom:             vNom,
    ...
    for (let i = 0; i < imagesCount; i++) {
      ...
      if (!url) continue;
      images.push({
        id:         data.get(`image_id_${i}`) ? parseInt(data.get(`image_id_${i}`) as string) : null,
        url,
    ...
    try {
      await withTransaction(async (client) => {
        ...
        // 1. Migrer images de la description temp → products/{id}
        const descriptionFinale = await migrateDescriptionImages(description, id);

        // 2. Update produit avec description finale
        await client.query(
          `UPDATE produits SET
             nom = $1, slug = $2, description = $3,
             reference = $4, categorie_id = $5, prix = $6,
             unite_vente = $7, stock = $8, stock_minimum = $9,
             actif = $10
           WHERE id = $11`,
          [nom, slug, descriptionFinale, reference, categorie_id,
           prix, unite_vente, stock, stock_minimum, actif, id]
        );

        // 3. Mettre à jour prix B2B
        ...
        for (const tc of typesClient.rows) {
          ...
          await client.query(
            `INSERT INTO prix_client (produit_id, type_client_id, prix)
             VALUES ($1, $2, $3)
             ON CONFLICT (produit_id, type_client_id)
             DO UPDATE SET prix = $3`,
            [id, tc.id, prixB2B]
          );
        }

        // 4. Supprimer et réinsérer variantes
        await client.query('DELETE FROM variantes WHERE produit_id = $1', [id]);
        ...
        for (const v of variants) {
          await client.query(
            `INSERT INTO variantes
             ...
             VALUES ($1,$2,$3,$4,$5,$6)`,
            [id, v.nom, v.reference, v.prix_supplement, v.stock, v.actif]
          );
        }

        // 5. Supprimer anciennes images galerie et réinsérer
        await client.query('DELETE FROM produit_images WHERE produit_id = $1', [id]);

        const destDir = path.join(UPLOADS_DIR, 'products', String(id));
        if (!existsSync(destDir)) await mkdir(destDir, { recursive: true });
        ...
        for (const img of images) {
          ...
          if (img.url.startsWith('/uploads/products/temp/')) {
            ...
            finalUrl = `/uploads/products/${id}/${filename}`;
          }

          await client.query(
            `INSERT INTO produit_images (produit_id, url, ordre, principale)
             VALUES ($1, $2, $3, $4)`,
            [id, finalUrl, img.ordre, img.principale]
          );
        }
