<script lang="ts">
  ...
  let { category = null, catalogues, action } = $props<{
    category?: {
      id?: number;
      nom: string;
      slug: string;
    ...
    catalogues: { id: number; nom: string }[];
    action: string;
  }>();

  let loading       = $state(false);
  let nom           = $state(category?.nom           ?? '');
  let slug          = $state(category?.slug          ?? '');
  let description   = $state(category?.description   ?? '');
  let catalogue_id  = $state(category?.catalogue_id  ?? catalogues[0]?.id ?? 0);
  let ordre         = $state(category?.ordre         ?? 0);
  let actif         = $state(category?.actif         ?? true);
  let slugEdited    = $state(!!category);
  ...
  <div class="card-body p-4 lg:p-6">
    ...
    >
      ...
      <div class="form-control gap-1.5">
        <label class="label py-0" for="catalogue_id">
          <span class="label-text font-medium">
            Parent Catalog <span class="text-error">*</span>
          </span>
        </label>
        <select
          id="catalogue_id"
          ...
          bind:value={catalogue_id}
          required
        >
          {#each catalogues as catalog}
            <option value={catalog.id}>{catalog.nom}</option>
          {/each}
        </select>
      </div>
      ...
      <div class="flex flex-col sm:flex-row justify-end gap-3">
        <a href="/dashboard/categories" class="btn btn-ghost w-full sm:w-auto">
          Cancel
        </a>
        <button
          type="submit"
